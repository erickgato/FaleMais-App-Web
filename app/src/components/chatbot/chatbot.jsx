...
export default class ChatBot extends React.Component {
    ...
    Data.GetResults()
      .then((results) => {
        ...
        Object.values(results).forEach((val, index) => {
          ...
          preparedArray.push(SplitFinalMessage[index].replace("$I", val));
        });
        PreparedString = preparedArray.join('</br>')
        const LastMessage = {
          Qname: "LastMessage",
          question: PreparedString,
          id: "L-M",
          splitdata: ArrayResults,
          options: [
            {
              content: "Denovo!",
              id: 40,
  ...
  AwaitMessage() {
    ...
    if (Msg.length === 0) this.DueTheFinalMessage();
    else {
      this.setState((state) => {
  ...
  RenderOptions() {
    ...
    const Datasets = this.state.Bot.passedmessages[Index - 1].map(
      (optvalue) => {
      ...
      return option.map((item, i) => {
        return (
          <button
            className="botoption"
            key={`opt-${i}`}
            onClick={() => this.setOptionClick(item.content, item.id)}
          >
            {" "}
            {item.content}{" "}
          </button>
    ...
    return (
      <React.Fragment>
        <input
          ...
          min='1'
          id="messageinput"
          value={this.state.People.input}
          type="number"
          onChange={(v) => this.Setinput(v.target.value)}
          onKeyPress={this.SubmitButton}
          placeholder="Digite o valor..."
          autoFocus
        />
        <button className="btn-send" key={`b-${this.key}`} onClick={() => this.PushData()}>
          <img key={`img-${this.key}`} src={sendicon} alt="enviar" />
        </button>
      </React.Fragment>
    );
  }
  ...
  RenderMessagesInputs() {
    ...
    if (Data.toString() === "") return this.RenderInputs();
    else return this.RenderOptions();
  }
  /**
  ...
  RenderMessages() {
    ...
    this.state.Bot.passedmessages.map((message) => {
      return message.map((value) => {
        if(value.id === 'L-M')
          return Messages.Bot.push(<Bot key={`Bot-${value.id}`} table={value.splitdata}  />)
        else
        return Messages.Bot.push(<Bot key={`Bot-${value.id}`} message={value.question} />);
      });
    });
    this.state.messages.map((msg) => {
      ++this.key;
     return Messages.People.push( <People key={`Pep-${this.key}`} message={msg} />);
    });
    Messages.Bot.map((Mjsx,index) => {
        return Messages.Total.push(Mjsx,Messages.People[index])
    })
    ...
    return (
      <React.Fragment>
        <div className="chat">
          ...
          <div className="messageinput">{this.RenderMessagesInputs()}</div>
        </div>
      </React.Fragment>
    );
  }
